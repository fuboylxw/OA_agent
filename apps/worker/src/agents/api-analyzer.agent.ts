...
办事流程的特征：
- 用户可以发起的业务操作（如发起某类申请、提交表单、办理事项）
- 用户可以查询的业务状态（如我的待办、我的已办、申请状态、办理记录）
...
- 如果提供了 responseFields，请据此推断 responseMapping`;
    ...
    if (allProcesses.length === 0) {
      this.logger.warn(
        `LLM returned no business processes for ${operations.length} operations; skipping heuristic workflow guessing`,
      );
    }
