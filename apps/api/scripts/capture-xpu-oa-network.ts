...
function shouldCapture(url: string) {
  return url.includes('/gate/auth/oauth/token')
    || url.includes('/gate/lobby/api/oa/info')
    || url.includes('/gate/lobby/api/v1/hotApps')
    || url.includes('/gate/static/api/v1/show/');
}
