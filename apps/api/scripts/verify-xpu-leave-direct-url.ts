...
async function main() {
  ...
  try {
    ...
    const cookiesAfterPortal = await context.cookies();
    ...
    const screenshotPath = path.join(outputDir, `leave-direct-url-${Date.now()}.png`);
    ...
    console.log(JSON.stringify({
      ...
      bodyPreview: (await page.locator('body').innerText().catch(() => '')).replace(/\s+/g, ' ').trim().slice(0, 1000),
      cookiesAfterPortal: cookiesAfterPortal
        .filter((cookie) => /xpu|seeyon|oa2023|token|session/i.test(cookie.domain || '') || /token|session|jsessionid|ticket/i.test(cookie.name))
        .map((cookie) => ({
          name: cookie.name,
