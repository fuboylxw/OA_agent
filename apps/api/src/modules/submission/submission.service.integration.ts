...
describe('SubmissionService Integration', () => {
  ...
  const mockDeliveryOrchestratorService = {
    submit: jest.fn(),
    queryStatus: jest.fn(),
  };
  ...
  const mockAttachmentService = {
    prepareSubmissionPayload: jest.fn().mockResolvedValue({}),
  };
  const mockAttachmentBindingService = {
    syncSubmissionBindings: jest.fn().mockResolvedValue(undefined),
  };
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: DeliveryOrchestratorService, useValue: mockDeliveryOrchestratorService },
        { provide: ChatSessionProcessService, useValue: mockChatSessionProcessService },
        { provide: AttachmentService, useValue: mockAttachmentService },
        { provide: AttachmentBindingService, useValue: mockAttachmentBindingService },
  ...
  it('creates a submission, persists creation event, updates draft, and enqueues async execution', async () => {
    ...
    expect(mockQueue.add).toHaveBeenCalledWith('execute', expect.objectContaining({
      submissionId: 'submission-1',
      connectorId: 'connector-1',
      processCode: 'travel_expense',
      formData: {
