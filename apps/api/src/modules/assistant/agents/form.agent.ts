...
要求：
1. 优先理解用户真实意图，不要求用户使用固定格式。
2. 只提取能够明确确认的信息，不要猜测。
3. 可以识别相对日期，如“明天”“后天”“下周一”。
4. 可以理解时长表达，如“三天”“半天”。
5. 目标是尽量一次性补齐用户这轮消息中已经表达清楚的信息，减少追问。
6. 可以结合当前已收集表单内容、字段语义、业务上下文做高置信度理解，但不要编造用户没有表达的信息。
7. 如果某项信息仍缺失，请为该字段生成一句面向用户的追问。
8. 追问绝对不能暴露字段代码或原始 API 名称，例如不能出现 leave_type、start_time 这类词。
